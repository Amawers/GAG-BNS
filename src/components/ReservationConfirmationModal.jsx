      ...
      <div className="modal-dialog">
        <div className="modal-content">
          ...
          <div className="modal-body">
            ...
            />
            <ul className="list-group mb-2" style={{ maxHeight: "150px", overflowY: cart.length > 3 ? "auto" : "visible" }}>
              {cart.map((item) => (
                <li key={item.id} className="list-group-item d-flex justify-content-between">
                  <span>{item.product_name} x {item.qty} (₱{item.price_each})</span>
          ...
          <div className="modal-footer">
            ...
            <button className="btn btn-warning" onClick={handleReserve}>Reserve</button>
          </div>
        </div>
      </div>
    </div>
